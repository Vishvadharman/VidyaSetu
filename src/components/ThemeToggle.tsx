...
export default function MainLayout({ children }: PropsWithChildren) {
  ...
  return (
    <div className="flex w-screen">
      <aside className="min-h-screen w-15 bg-accent/40 pt-8 text-foreground">
        <div className="fixed flex w-15 flex-col gap-4">
          {elements.map((val) => (
            <a
              key={val.name}
              href={val.link}
              aria-label={val.name}
              className={`flex cursor-pointer items-center justify-center p-4 transition-colors ${
                path.startsWith(val.link)
                  ? 'border-r-4 border-r-primary bg-card text-card-foreground'
